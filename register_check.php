...
$query_clients = <<<EOD
INSERT INTO clients (name, address, phone, email, birthdate) VALUES ('{$name}', '{$address}', '{$phone}', '{$email}', '{$birth}');
EOD;
...
if (mysqli_query($conn, $query_clients)) {
	...
	$query_users = <<<EOD
	INSERT INTO users (username, password, id_client) VALUES ('{$user}', '{$password_db}', {$id_client});
EOD;
